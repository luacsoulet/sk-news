...
# --- 2. EXTRACTION SPÉCIFIQUE (Les Echos) ---

def extraire_donnees_les_echos(html_content, url, source_id):
    ...
    # 1. Extraction prioritaire via JSON-LD (Très présent sur Les Echos)
    json_ld_script = soup.find('script', id='articleJsonLd')
    if not json_ld_script:
        # Fallback au cas où l'ID change mais que le type reste
        json_ld_script = soup.find('script', type='application/ld+json', string=re.compile(r'"@type"\s*:\s*"NewsArticle"'))
    ...
    if json_ld_script:
        try:
            ...
            if "articleBody" in data:
                article_data["content"] = structurer_le_corps(nettoyer_texte(data["articleBody"]))
        except Exception as e:
            print(f"Erreur de lecture du JSON-LD : {e}")

    # 2. Complétion via HTML (Si le JSON-LD est incomplet ou bloqué)
    if not article_data["content"] or len(article_data["content"]) < 100:
        ...
        if not article_data["description"]:
            lead = soup.find('p', attrs={'data-testid': 'post-lead'})
            if lead: article_data["description"] = nettoyer_texte(lead.get_text())
            
        # Corps de l'article dans la structure React
        corps_final = []
        body_container = soup.find('div', class_=re.compile(r'post-paywall'))
        ...
        if meta_date and article_data.get("created_at") == datetime.now().isoformat():
            try:
                dt = datetime.fromisoformat(meta_date['content'].replace('Z', '+00:00'))
                article_data["created_at"] = dt.strftime("%Y-%m-%dT%H:%M:%S")
            except: pass
            
        if soup.find('meta', attrs={'name': 'ad:postAccess', 'content': 'paywall'}):
            article_data["is_paywall"] = True
...
def process_les_echos(google_url, output_filename, source_id=1):
    print(f"🚀 Lancement Furtif (Les Echos) : {google_url}")
    
    with sync_playwright() as p:
        # ANTI-BOT 1 : Le flag magique qui désactive la détection d'automatisation
        browser = p.chromium.launch(
            headless=True,
            args=["--disable-blink-features=AutomationControlled"] 
        ) 
        
        # ANTI-BOT 2 : Un profil 100% humain (Headers + User Agent parfaits)
        ...
        # ANTI-BOT 3 : Masquer la signature "webdriver" de Playwright
        page.add_init_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined})")

        # Bloquer les ressources lourdes pour aller vite, mais laisser les scripts JS (obligatoire pour Les Echos)
        ...
        try:
            ...
            # Attente de la redirection (Sortie de Google News)
            ...
            # Sécurité : on attend le titre de l'article pour être sûr de ne pas être sur la page "Access Denied"
            try:
                page.wait_for_selector("h1, .post-paywall", timeout=10000)
            except:
                time.sleep(2)
            
            # Extraction finale
            resultat = extraire_donnees_les_echos(page.content(), normalize_url(page.url), source_id)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        process_les_echos(url, output_file, source_id)