...
# --- 2. EXTRACTION SPÉCIFIQUE (Ukraïner) ---

def extraire_donnees_ukrainer(html_content, url, source_id):
    ...
    h1 = soup.find('h1', class_='headline') or soup.find('h1')
    if h1: 
    ...
    # Le contenu est réparti dans des blocs avec la classe 'text-section'
    text_sections = soup.find_all('div', class_=re.compile(r'text-section'))
    
    for section in text_sections:
        # On exclut les sections de fin d'article (comme les auteurs ou "Soutenir le projet")
        if "authors" in section.get('class', []) or "r-padding-null" in section.get('class', []):
             continue

        # On récupère tous les paragraphes et sous-titres
        for p in section.find_all(['p', 'h2', 'h3']):
             # On ignore les paragraphes qui sont juste des légendes d'images ou des liens
             if "sm-text" in p.get('class', []) or p.get('id') == "breadcrumbs":
                 continue

             txt = nettoyer_texte(p.get_text())
             if len(txt) > 5:
                 # Si c'est un sous-titre, on le met en forme
                 if p.name in ['h2', 'h3']:
                     corps_final.append(f"## {txt}")
                 else:
                     # On gère l'espace manquant lié au span de la première lettre (ex: "U n groupe")
    ...
    for paragraphe in corps_final:
        if paragraphe not in corps_final_sans_doublons:
            corps_final_sans_doublons.append(paragraphe)
...
def process_ukrainer(google_url, output_filename, source_id=1):
    print(f"🚀 Lancement Flash (Ukraïner) : {google_url}")
    ...
    try:
        with sync_playwright() as p:
            ...
            # Sécurité : On attend que le titre apparaisse
            try:
                page.wait_for_selector(".headline", timeout=10000)
                time.sleep(1) 
            except Exception:
                print("⚠️ Attention : La zone de texte a mis trop de temps à charger.")

            # Extraction
            resultat = extraire_donnees_ukrainer(page.content(), normalize_url(page.url), source_id)
            browser.close()
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        process_ukrainer(url_cible, fichier_sortie, id_source)
    else:
