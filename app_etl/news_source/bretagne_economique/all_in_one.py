...
# --- 2. EXTRACTION SPÉCIFIQUE (Bretagne Économique) ---

def extraire_donnees_bretagne_eco(html_content, url, source_id):
    ...
    # Le contenu est contenu dans la section avec l'itemprop "text"
    article_container = soup.find('section', attrs={'itemprop': 'text'}) or soup.find('div', class_='entry-content')
    
    if article_container:
        # On essaie d'abord de récupérer le chapeau (intro)
        chapeau = article_container.find('div', id='chapeau')
        if chapeau:
            txt_chap = nettoyer_texte(chapeau.get_text())
            if txt_chap:
                corps_final.append(txt_chap)
            chapeau.decompose() # On le retire pour ne pas le lire deux fois
            
        # Nettoyage des éléments parasites (pubs, iframes, scripts)
        for unwanted in article_container.find_all(['script', 'style', 'iframe']):
            unwanted.decompose()
...
def process_bretagne_eco(google_url, output_filename, source_id=1):
    print(f"🚀 Lancement Flash (Bretagne Éco) : {google_url}")
    ...
    try:
        with sync_playwright() as p:
            ...
            # Sécurité : On attend que l'article apparaisse
            try:
                page.wait_for_selector(".entry-content, [itemprop='text']", timeout=10000)
                time.sleep(1) 
            except Exception:
                print("⚠️ Attention : La zone de texte a mis trop de temps à charger.")

            # Extraction
            resultat = extraire_donnees_bretagne_eco(page.content(), normalize_url(page.url), source_id)
            browser.close()
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        process_bretagne_eco(url_cible, fichier_sortie, id_source)
    else:
